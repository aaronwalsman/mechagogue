...
from mechagogue.game_solvers.ippo import IPPOParams, make_ippo
...
def _make_report(state, losses, stats=None):
    loss_per_player = jnp.stack([jnp.mean(l) for l in losses], axis=0)
    loss = jnp.mean(loss_per_player)
    if stats is None:
    ...
    report = {
        "loss_mean": loss,
        "loss_per_player": loss_per_player,
...
def _log(key, epoch, state, reports):
    ...
    loss_per_player = jax.device_get(reports["loss_per_player"])
    loss_per_player = [float(jnp.mean(x)) for x in loss_per_player]
    ...
    loss_str = " ".join(
        [f"p{i}_loss {v:.6f}" for i, v in enumerate(loss_per_player)]
    )
    print(
        f"epoch {epoch} loss_mean {loss:.6f} {loss_str} "
...
def run_training(
    ...
    output_directory="training_runs/nim_ippo",
):
    key = jrng.key(seed)

    ippo_params = IPPOParams(
    ...
    policy_a = make_nim_policy(max_pile=max_pile)
    policy_b = make_nim_policy(max_pile=max_pile)
    policies = [policy_a, policy_b]
    optimizers = [adam(learning_rate=3e-4), adam(learning_rate=3e-4)]

    ippo = make_ippo(ippo_params, env, policies, optimizers)
    ...
    for _ in range(epochs):
        ...
        win_a, wins_a, losses_a, draws_a = _eval_vs_optimal(
            eval_key,
            policies[0],
            state.system_state.model_states[0],
            max_pile,
            model_player=0,
        ...
        win_b, wins_b, losses_b, draws_b = _eval_vs_optimal(
            subkey,
            policies[1],
            state.system_state.model_states[1],
            max_pile,
            model_player=1,
            deterministic=eval_deterministic,
        )
